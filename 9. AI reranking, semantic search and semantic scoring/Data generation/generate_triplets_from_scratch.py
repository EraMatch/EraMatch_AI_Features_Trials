"""
generate_triplets_from_scratch.py  (v3)
========================================
Contrastive Learning dataset generator for Bi-Encoder training.

Pipeline per batch (produces up to 8 samples):
  1. Pick 2 tech domains (A, B).
  2. Generate 2 different HR queries per domain → 4 queries (A1, A2, B1, B2).
     A2 is generated with A1 in context to ensure diversity; same for B2/B1.
  3. Generate one positive CV per query → 4 CVs.
  4. Judge: verify each CV clearly satisfies its own query.
  5. Assemble 8 samples by cross-referencing:
       hard negative = CV from the OTHER query in the SAME domain
       easy negative = CV from a query in the OTHER domain
...
OLLAMA_API_KEY  = "ollama"            # any non-empty string for local Ollama

GENERATOR_MODEL = "qwen2.5-coder:7b"  # model for query + CV generation
JUDGE_MODEL     = "qwen2.5-coder:7b"  # model for CV-query matching judge
...
class SampleJudgeVerdict(BaseModel):
    matches: bool   # does the CV satisfy its query?
    reason:  str


# ═════════════════════════════════════════════════════════════════════════════
# JSON robustness — fix common local-LLM output quirks
# ═════════════════════════════════════════════════════════════════════════════
...
# ═════════════════════════════════════════════════════════════════════════════
# Type coercion — fix bool/float/str mismatches before Pydantic validation
# ═════════════════════════════════════════════════════════════════════════════
...
# ═════════════════════════════════════════════════════════════════════════════
# CV sanitization (date fixing, consistency enforcement)
# ═════════════════════════════════════════════════════════════════════════════
...
def generate_cv(query: str, name: str) -> Dict[str, Any]:
    ...
    raw = _chat(CV_SYSTEM, user, GENERATOR_MODEL, temperature=0.85)
...
# ═════════════════════════════════════════════════════════════════════════════
# Batch generation — 2 domains × 2 queries × 2 sample types = 8 samples
# ═════════════════════════════════════════════════════════════════════════════
def generate_batch() -> List[Dict[str, Any]]:
    """
    Pick 2 domains, generate 4 queries and 4 CVs, judge each CV against its query,
    then assemble 8 samples by cross-referencing within/across domains.
    Returns a list of 0 (batch aborted) or 8 approved sample dicts.
    """
    ...
    query_map  = {"a1": q_a1, "a2": q_a2, "b1": q_b1, "b2": q_b2}
    ...
    # Cross-judge the 4 intra-domain pairs. If any cross-match returns true the
    # two queries overlapped too much (e.g. both senior Go roles) and the "hard
    # negative" would actually satisfy the anchor — abort to prevent poisoning.
    samples = [
...
def main() -> None:
    print("=" * 68)
    print("  Triplet Dataset Generator — Local Ollama  (v3)")
    print(f"  Generator : {GENERATOR_MODEL}  |  Judge: {JUDGE_MODEL}")
    print(f"  Target    : {TARGET_COUNT:,} samples  |  Workers: {MAX_WORKERS}")
    print(f"  Batch     : 2 domains × 2 queries × 2 types = 8 samples/batch")
